...
void fillV(vector<int> &v ,Node* root, int l,int h)
{
    if(root == NULL) return;
    else if( root->data <= h &&  root->data >= l)
    {   fillV(v,root->left,l,h);
        v.push_back(root->data);
        
        fillV(v,root->right,l,h);
        
    }
    
    else if(root->data > h)fillV(v,root->left,l,h);
    else fillV(v,root->right,l,h);
}

vector<int> printNearNodes(Node *root, int low, int high)
{
  vector<int> v;
  fillV(v,root,low,high);
  return v;
}
