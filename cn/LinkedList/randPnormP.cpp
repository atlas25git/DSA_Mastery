...
Node *copyList(Node *head) {
    ...
    while(t!=NULL){
        if(t->arb == NULL)t->next->arb==NULL;
        else t->next->arb = t->arb->next;
        t = t->next->next;
    }
    t = head;
    ...
    while(t != NULL)
    {
        //t and head both at head, i.e., of list 1
        head = t->next->next;//moving head to the second node of list 1,
        //node 2 of list 2 can be accessed by ->next
        
        if(head)t->next->next = head->next;
        // in the 1st node of l2, we linked it to
       else t->next->next = NULL;
        
        //linking nodes of l1
        t->next = head;
        t = head;//to bring them at same place
    }
